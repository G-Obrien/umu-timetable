...
$row=mysqli_num_rows($que);
	if($row)
	{
	$err="<font color='red'>Student already exists</font>";
	}
	else
	{
	$image=$_FILES['pic']['name'];	
		
       mysqli_query($con,"insert into student               values('','$stname','$eid','$p','$mobile','$address','$courseid', '$c', '$s','$dob','$image','$gen','$status',now())");	

    mkdir("../student/image/$eid");
     move_uploaded_file($_FILES['pic']['tmp_name'],"../student/image/$eid/".$_FILES['pic']['name']);


	$err="<font color='blue'>Student Registration Saved!!!</font>";
	}
...
<table border="0" class="table">
  ...
  <td colspan="2"><?php echo @$err; ?></td>
    ...
    <!--Year of studyr--->
    <tr>
    <th width="237" scope="row">Select Year of Study</th>
    <td width="213">
	<select name="c" id="class" onchange="showsemester(this.value)" class="form-control">
  ...
  <tr>
    ...
    <td width="213"><input type="text" name="stname" class="form-control" placeholder="Enter student's name"/></td>
  </tr>
  <tr>
    <th scope="row">Enter Your Email </th>
    <td><input type="email" name="eid" class="form-control" placeholder="Enter student's email"/></td>
  </tr>
  
  <tr>
    <th scope="row">Enter Your Password </th>
    <td><input type="password" name="p" class="form-control" placeholder="Enter student's Password"/></td>
  </tr>
  
  <tr>
    <th scope="row">Enter Your Mobile </th>
    <td><input type="number" name="mobile" class="form-control" placeholder="Enter student's mobile"/></td>
  </tr>
  
  <tr>
    <th scope="row">Enter Your Address</th>
    <td><input type="text" name="address" class="form-control" placeholder="Enter  student's address"/></td>
  </tr>
  
   <tr>
    <th scope="row">Enter Your D.O.B</th>
    <td><input type="date" name="dob" class="form-control" placeholder="Enter student's D.O.B"/></td>
  </tr>
  
   <tr>
    <th scope="row">Upload Your Pic</th>
    <td><input type="file" name="pic" class="form-control" placeholder="Upload student's Pic"/></td>
  </tr>
